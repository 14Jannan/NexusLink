...
function InfoBoxCard({ icon, title, description, linkText, to }) {
    // This component correctly handles internal and external/scroll links
...
function AdvertisementCard({ companyName, position, message, contactPerson, contactEmail, contactPhone }) {
    return (
        <div className="card-container advertisement-card">
            ...
            <div className="skills advertisement-contact">
...
const AdvertisementList = () => {
    ...
    // MODIFIED: This useEffect now fetches data from the Ballerina backend.
    useEffect(() => {
        const fetchAdvertisementsFromBackend = async () => {
            try {
                // Call the /advertisements endpoint on our Ballerina server
                const dataFromBackend = await getData('advertisements');

                // Convert the object of objects from Firebase into a usable array
                ...
                // Sort the ads by date, newest first
